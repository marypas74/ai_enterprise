/**
 * useAgentStore — Unit tests for the Agent Zustand store.
 *
 * Tests cover:
 * - Initial state
...
describe('useAgentStore', () => {
  ...
  describe('initial state', () => {
    ...
    it('should have no selected session', () => {
      expect(useAgentStore.getState().selectedSession).toBeNull();
    });
    ...
    it('should have empty templates', () => {
      expect(useAgentStore.getState().templates).toEqual([]);
    });

    it('should have null orchestratorMetrics', () => {
      expect(useAgentStore.getState().orchestratorMetrics).toBeNull();
    });
  });
  ...
  describe('fetchSessions', () => {
    ...
    it('should pass query params to API', async () => {
      ...
      await useAgentStore.getState().fetchSessions({ status: 'running', limit: 10, offset: 0 });
    ...
    it('should use fallback error message when no API error provided', async () => {
      ...
      expect(useAgentStore.getState().error).toBe('Failed to fetch sessions');
    });
  });
  ...
  describe('fetchSession', () => {
    it('should set selectedSession from API', async () => {
      ...
      const result = await useAgentStore.getState().fetchSession(1);

      expect(useAgentStore.getState().selectedSession).toEqual(mockSession);
    ...
    it('should return null on error', async () => {
      ...
      const result = await useAgentStore.getState().fetchSession(999);

      expect(result).toBeNull();
      expect(useAgentStore.getState().error).toBe('Not found');
    });
  });
  ...
  describe('createSession', () => {
    it('should add new session to the list', async () => {
      ...
      const result = await useAgentStore.getState().createSession({
        name: 'New Session',
      ...
      expect(useAgentStore.getState().sessions).toContainEqual(newSession);
      expect(useAgentStore.getState().isLoading).toBe(false);
    });
    ...
    it('should set error and throw on failure', async () => {
      ...
      expect(useAgentStore.getState().error).toBe('Validation failed');
    });
  });
  ...
  describe('deleteSession', () => {
    ...
    it('should keep selectedSession if different session is deleted', async () => {
      ...
      expect(useAgentStore.getState().selectedSession).toEqual({ id: 2 });
    });
  });
  ...
  describe('updateSession', () => {
    it('should update session in the list', async () => {
      ...
      await useAgentStore.getState().updateSession(1, { name: 'New Name' });
  ...
  describe('fetchTemplates', () => {
    it('should set templates from API', async () => {
      ...
      await useAgentStore.getState().fetchTemplates();

      expect(useAgentStore.getState().templates).toEqual(mockTemplates);
    });

    it('should pass category param when provided', async () => {
      mockApi.get.mockResolvedValueOnce({ data: [] } as any);

      await useAgentStore.getState().fetchTemplates('coding');
  ...
  describe('deleteTemplate', () => {
    it('should remove template from the list', async () => {
      ...
      await useAgentStore.getState().deleteTemplate(1);

      expect(useAgentStore.getState().templates).toHaveLength(1);
      expect(useAgentStore.getState().templates[0].id).toBe(2);
    });
  });
  ...
  describe('fetchSessionLogs', () => {
    it('should set sessionLogs from API', async () => {
      ...
      await useAgentStore.getState().fetchSessionLogs(1);

      expect(useAgentStore.getState().sessionLogs).toEqual(mockLogs);
    });
  });
  ...
  describe('fetchTerminalSlots', () => {
    it('should set terminalSlots from API', async () => {
      ...
      await useAgentStore.getState().fetchTerminalSlots();

      expect(useAgentStore.getState().terminalSlots).toEqual(mockSlots);
    });
  });
  ...
  describe('fetchOrchestratorMetrics', () => {
    it('should set orchestratorMetrics from API', async () => {
      ...
      await useAgentStore.getState().fetchOrchestratorMetrics();

      expect(useAgentStore.getState().orchestratorMetrics).toEqual(mockMetrics);
    });
  });
  ...
  describe('UI actions', () => {
    it('setSelectedSession should update selectedSession', () => {
      ...
      useAgentStore.getState().setSelectedSession(session);

      expect(useAgentStore.getState().selectedSession).toEqual(session);
    });

    it('setSelectedSession(null) should clear selectedSession', () => {
      useAgentStore.setState({ selectedSession: { id: 1 } as any });
      useAgentStore.getState().setSelectedSession(null);
