...
export class ApiService {
    ...
    public async login(credentials: ILoginRequest): Promise<ILoginResponse> {
        ...
        if (setCookie) {
            const refreshMatch = setCookie.find((c: string) => c.startsWith('refreshToken='));
            if (refreshMatch) {
    ...
    public async getCurrentUser(): Promise<IUser> {
        const response = await this.axiosInstance.get<IUser>('/api/auth/me');
    ...
    public async getModels(): Promise<IModel[]> {
        const response = await this.axiosInstance.get<IModel[]>('/api/chat/models');
        return response.data;
    }
    ...
    public async getConversations(
        ...
        const response = await this.axiosInstance.get<IConversation[]>(
    ...
    public async getConversationMessages(
        ...
        const response = await this.axiosInstance.get<IConversationWithMessages>(
            ...
            while (true) {
                ...
                for (const line of lines) {
                    if (line.startsWith('data: ')) {
                        ...
                        if (jsonStr) {
                            try {
                                const chunk: IStreamChunk = JSON.parse(jsonStr);
                                onChunk(chunk);
    ...
    private logError(error: AxiosError): void {
        ...
        if (status === 404) {
            this.log(
                'HINT: 404 error - Check that the URL includes /api prefix. ' +
                    'Ingress routes /api/* to backend.'
            );
        } else if (status === 502) {
            this.log(
                'HINT: 502 Bad Gateway - Backend pods may be down or restarting.'
            );
        }
    }
}
