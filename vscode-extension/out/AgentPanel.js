...
exports.AgentApiService = exports.AgentDashboardProvider = exports.TerminalSlotsProvider = exports.AgentSessionsProvider = void 0;
...
class AgentSessionsProvider {
    ...
    getChildren(element) {
        ...
        return Promise.resolve(this.sessions.map(session => new AgentSessionItem(session)));
    }
    getSession(id) {
        return this.sessions.find(s => s.id === id);
    }
}
exports.AgentSessionsProvider = AgentSessionsProvider;
...
class TerminalSlotsProvider {
    ...
    getChildren(element) {
        ...
        return Promise.resolve(allSlots.map(slot => new TerminalSlotItem(slot)));
    }
}
exports.TerminalSlotsProvider = TerminalSlotsProvider;
...
class AgentDashboardProvider {
    ...
    _view;
    api;
    constructor(_extensionUri, serverUrl, accessToken) {
    ...
    _getHtmlForWebview(webview, metrics) {
...
exports.AgentApiService = AgentApiService;
// ============================================
// REGISTER AGENT PANEL COMMANDS
// ============================================
function registerAgentCommands(context, sessionsProvider, slotsProvider, dashboardProvider, agentApi) {
    // Refresh agents
    context.subscriptions.push(vscode.commands.registerCommand('enterprise-ai-chat.refreshAgents', async () => {
        await Promise.all([
    ...
    context.subscriptions.push(vscode.commands.registerCommand('enterprise-ai-chat.createAgentSession', async () => {
    ...
    // Show session detail
    context.subscriptions.push(vscode.commands.registerCommand('enterprise-ai-chat.showSessionDetail', async (session) => {
        const panel = vscode.window.createWebviewPanel('agentSessionDetail', `Session: ${session.name}`, vscode.ViewColumn.One, { enableScripts: true });
        // Get logs
        let logs = [];
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('enterprise-ai-chat.startSession', async (item) => {
        await agentApi.startSession(item.session.id);
        sessionsProvider.refresh();
        slotsProvider.refresh();
    }), vscode.commands.registerCommand('enterprise-ai-chat.pauseSession', async (item) => {
        await agentApi.pauseSession(item.session.id);
        sessionsProvider.refresh();
    }), vscode.commands.registerCommand('enterprise-ai-chat.resumeSession', async (item) => {
        await agentApi.resumeSession(item.session.id);
        sessionsProvider.refresh();
    }), vscode.commands.registerCommand('enterprise-ai-chat.cancelSession', async (item) => {
        await agentApi.cancelSession(item.session.id);
        sessionsProvider.refresh();
        slotsProvider.refresh();
    }));
}
function getSessionDetailHtml(session, logs) {
    const statusColors = {
        pending: '#808080',
        initializing: '#2196f3',
        running: '#4caf50',
