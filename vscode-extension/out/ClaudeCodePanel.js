...
class ClaudeCodePanel {
    static currentPanel;
    static viewType = 'claudeCode.panel';
    _panel;
    _extensionUri;
    _disposables = [];
    // State
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel(ClaudeCodePanel.viewType, 'Enterprise AI', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _handleMessage(message) {
        switch (message.type) {
            case 'send':
                ...
                vscode.commands.executeCommand('enterprise-ai-chat.sendMessage', message.message);
                break;
            case 'login':
            ...
            case 'logout':
                ...
                break;
            case 'selectModel':
                vscode.commands.executeCommand('enterprise-ai-chat.selectModel', message.modelId);
                break;
            case 'runCommand':
            ...
            case 'sendAgentic':
                ...
                if (message.message && message.projectId !== undefined) {
                    vscode.commands.executeCommand('enterprise-ai-chat.sendAgenticMessage', message.message, message.projectId);
                }
                else {
    ...
    _getHtmlForWebview(webview) {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'out', 'claudeCodeWebview.js'));
        // Get the CSS URI
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'out', 'claudeCode.css'));
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
