      ...
      try {
        ...
        if (modelConfig.supportsThinking) {
          if (isAnthropic) {
            ...
            // For qwen3 models: ALWAYS strip tools when thinking is desired,
            // because tools + think: true is fundamentally broken in Ollama.
            // For other Ollama models: disable thinking when tools are present.
            const isQwen3 = /qwen3/i.test(body.model);
            if (isQwen3 && toolDefs && toolDefs.length > 0) {
              fastify.log.info(`[Chat] Ollama qwen3: stripping tools to enable thinking (ollama#10976/#11381 — tools+thinking=empty)`);
              toolDefs = undefined;
