...
- Use relative paths like "output/translated.docx".`;
      ...
      let autoGenerateDoc: false | 'docx' | 'pptx' | 'xlsx' | 'pdf' = false;
      {
        ...
        // Inject system prompt for ALL models when document generation is detected
        // Tool-supporting models get a hint to use the tool; non-tool models get full instructions
        if (autoGenerateDoc) {
          ...
          if (supportsTools) {
            // For tool-supporting models: instruct to use the generate tool, with structured fallback
...
Il sistema genererà automaticamente il ${formatName} dalla tua risposta.`;
            ...
            // For non-tool models: full instructions to write structured content
...
5. Concentrati SOLO sul contenuto: traduci/elabora tutto il testo e scrivilo nella risposta${pptxExtra}`;
      ...
      try {
              ...
              switch (autoGenerateDoc) {
                case 'pptx': {
                  ...
                  if (headings.length > 0) {
                    // Content before first heading (if any) becomes intro slide
                    ...
                    // No headings: split by triple newlines or large paragraphs
        ...
        if (isRetriable && !isParlantAgent && !(request as any)._fallbackAttempted) {
          ...
          if (isOllama && !fallbackMap[body.model]) {
            try {
              const lightRow = await findOne<any>(fastify.db,
                `SELECT m.model_id FROM ai_models m
                 JOIN ai_providers p ON m.provider_id = p.id
                 WHERE p.name = 'ollama' AND m.is_enabled = TRUE AND m.model_id != ?
          ...
          if (fallbackModel) {
            ...
            try {
              ...
              const fbStream = fbProvider.streamComplete({
                model: fallbackModel,
                messages,
                maxTokens: fbConfig.maxOutputTokens,
            ...
            if (errorMessage.includes('timeout')) userMessage = 'Request timed out. The AI service took too long to respond.';
            else if (errorMessage.includes('ECONNREFUSED') || errorMessage.includes('fetch failed')) userMessage = 'Could not connect to the AI service. Please try again later.';
            reply.raw.write(`data: ${JSON.stringify({ error: userMessage, done: true })}\n\n`);
            reply.raw.end();
            return;
          }
        } else {
          // Non-retriable error or Parlant error
          let userMessage = 'An error occurred while processing your request.';
      ...
      // Save assistant message (with AI Act metadata)
      const assistantMsgId = await insertOne(
        fastify.db,
      ...
      try {
        ...
        fastify.log.warn(`[Chat] Component token tracking failed: ${compErr.message}`);
      }
      ...
      // Send done:true AFTER all post-processing (auto-doc generation, auto-save, hooks)
      // so the frontend receives download links and notifications before closing the reader
  ...
  fastify.get('/models/recommended', {
    ...
    // Count active sessions (logged_out_at IS NULL + activity within 15 min)
      ...
      while (iteration < maxIterations && !requestAborted) {
        ...
        try {
          ...
          if (response.content) {
            ...
            // Fix AI-hallucinated download links: replace any /api/tools/download/ links
            // with the real download URLs from tool results
            if (generatedDownloads.length > 0) {
              content = content.replace(
                /\[([^\]]*)\]\(\/api\/tools\/download\/[^)]+\)/g,
                () => {
                  // Use the most recent generated download
