...
import { readdir, stat, createReadStream, promises as fsPromises } from 'fs';
import { join } from 'path';
import { promisify } from 'util';
import { MetricsService } from '../../services/MetricsService.js';

// DEBT-88-C: CA root path for browser trust import (read-only public endpoint).
// In container: /app/projects/certs/ (mount of host /data/shared-projects/).
// On host: /data/shared-projects/certs/. Resolved via INTERNAL_CA_PATH env override.
const INTERNAL_CA_PATH = process.env.INTERNAL_CA_PATH
  || join(process.env.PROJECTS_PATH || '/app/projects', 'certs', 'enterprise-ai-ca.crt');
...
export async function publicRoutes(fastify: FastifyInstance) {
    ...
    // DEBT-88-C: download the Internal CA certificate for browser trust import.
    // The CA is generated by scripts/generate-ca-and-cert.sh and saved to a
    // Samba-shared path. This endpoint exposes the public .crt (no auth required).
    fastify.get('/internal-ca.crt', {
        config: {
            rateLimit: {
                max: 20,
                timeWindow: '1 minute',
        ...
        try {
            const caContent = await fsPromises.readFile(INTERNAL_CA_PATH, 'utf-8');
            return reply
                .type('application/x-x509-ca-cert')
                .header('Content-Disposition', 'attachment; filename="enterprise-ai-ca.crt"')
                .send(caContent);
        } catch (err) {
            fastify.log.warn({ err }, '[internal-ca] CA file not found at ' + INTERNAL_CA_PATH);
            return reply.status(404).send({ error: 'Internal CA not yet generated. Run scripts/generate-ca-and-cert.sh' });
        }
    });
